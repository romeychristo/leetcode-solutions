...
function buildLinkedList(values: number[]): ListNode | null {
  ...
  const head = new ListNode(values[0]); // if array not empty, assign first value of array as head
  let current = head; // current = (val:1, next:null)
...
function mergeTwoLists(
  list1: ListNode | null, // leetcode given function, only takes list node so make sure to convert the array first
  list2: ListNode | null
): ListNode | null {
  const dummy = new ListNode(-1); //dummy is the first node, we wont return this, we'll return dummy.next so that we dont get -1. Any value can be assigned to dummy, technically.
  let current = dummy; // current = (val: -1,next:null)
...
const arr2: number[] = [1, 3, 5];

const l1: ListNode | null = buildLinkedList(arr1);
const l2: ListNode | null = buildLinkedList(arr2);

const answer: ListNode | null = mergeTwoLists(l1, l2);
